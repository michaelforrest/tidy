package tidy.mvc.collection {
	...
	import tidy.mvc.model.IEventMapper;
	...
	public dynamic class Collection extends Array implements IEventMapper{
		...
		private function dispatchDeselected():void{event_mapper.dispatchEvent(new Event(DESELECTED));}
...
//					_index = getIndex(current());
		...
		public function insert(object : Object, index : Number) : void{
...
//		}
		public function findAll(query : String, options : Object = null) : Collection {
			var results : Collection = new Collection();
			try{
				for (var i : int = 0; i < this.length; i++) {
					var item : Matchable = this[i];
					if(!item is Matchable) throw new Error(item + " not " + Matchable);
					if(item.matches(query)) {
						results.add(item);
						if(options && options.limit &&   results.length>=options.limit) break;
					}
				}
			} catch(e :Error){
